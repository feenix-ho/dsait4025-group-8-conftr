...
from experiments.run_emnist_byclass import (
    get_parameters as get_emnist_byclass_parameters,
)
...
flags.DEFINE_string("experiment_experiment", "", "experiments to run on dataset")
flags.DEFINE_integer("experiment_seeds", 10, "number of seed to run per experiment")
flags.DEFINE_boolean("experiment_debug", False, "debug experiment sweep and seeds")
...
def get_parameters(
    ...
    config.coverage.temperature = 1.0
    ...
    config.coverage.cross_entropy_weight = 0.0
    config.coverage.size_loss = "valid"
    config.coverage.size_transform = "identity"
    config.coverage.size_bound = 3.0
    ...
    config.conformal.cross_entropy_weight = 0.0
    config.conformal.size_loss = "valid"
    config.conformal.size_transform = "identity"
    config.conformal.size_bound = 3.0
...
def main(argv):
    ...
    logging.info(
        ...
        FLAGS.experiment_dataset,
        FLAGS.experiment_experiment,
    ...
    config.finetune.path = os.path.join(FLAGS.experiment_path, config.finetune.path)
